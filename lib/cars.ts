...
async function addImage({
  ...
  const response = await axiosPost<{ url: string }, CarImage>(
    `/cars/${carId}/images`,
    { url },
  );
...
async function getUnavailableDates(carId: number): Promise<UnavailableDate[]> {
  const response = await axiosGet<UnavailableDate[]>(
...
async function addUnavailableDate({
  ...
  >(`/cars/${carId}/unavailable-dates`, {
    date,
    ...(endDate ? { startDate: date, endDate } : {}),
  });
  return response.data as UnavailableDate;
}

async function deleteUnavailableDate({
  carId,
  dateId,
}: {
  carId: number;
...
export function useCarImages(carId: number, enabled: boolean) {
  return useQuery({
    queryKey: ["cars", carId, "images"],
    queryFn: () => getImages(carId),
  ...
  return useMutation({
    ...
    onSuccess: (_, variables) =>
      queryClient.invalidateQueries({
        queryKey: ["cars", variables.carId, "images"],
      }),
  });
}
...
export function useUnavailableDates(carId: number, enabled: boolean) {
  return useQuery({
    queryKey: ["cars", carId, "unavailable-dates"],
    queryFn: () => getUnavailableDates(carId),
    enabled: enabled && !!carId,
  });
}
export function useAddUnavailableDate() {
  const queryClient = useQueryClient();
  return useMutation({
    mutationFn: addUnavailableDate,
    onSuccess: (_, variables) =>
      queryClient.invalidateQueries({
        queryKey: ["cars", variables.carId, "unavailable-dates"],
      }),
  });
}
