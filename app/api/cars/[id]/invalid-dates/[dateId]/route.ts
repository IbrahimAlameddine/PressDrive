...
export async function DELETE(
  ...
  try {
    ...
    if (
      ...
      carId <= 0 ||
      !Number.isInteger(unavailableDateId) ||
      unavailableDateId <= 0
    ) {
      return NextResponse.json(
        { status: 400, message: "Invalid car or date ID." },
        { status: 400 },
      );
    }
    ...
    const unavailableDate = await prisma.unavailableDate.findFirst({
      where: { id: unavailableDateId, carId },
    });
    if (!unavailableDate)
      return NextResponse.json(
        { status: 404, message: "Unavailable date not found." },
        { status: 404 },
      );
