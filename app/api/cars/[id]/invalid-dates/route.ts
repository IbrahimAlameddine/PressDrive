  ...
  try {
    ...
    if (!carId)
      return NextResponse.json(
        { status: 400, message: "Invalid car ID." },
        { status: 400 },
      );
...
export async function POST(
  ...
  try {
    ...
    if (!dateRange)
      return NextResponse.json(
        { status: 400, message: "A valid date or date range is required." },
        { status: 400 },
      );

    const duplicate = await prisma.unavailableDate.findFirst({
      where: {
        carId,
    ...
    if (duplicate)
      return NextResponse.json(
        { status: 409, message: "This unavailable date already exists." },
        { status: 409 },
      );
    const unavailableDate = await prisma.unavailableDate.create({
      data: { car: { connect: { id: carId } }, ...dateRange },
    });
    return NextResponse.json(
      { status: 201, data: unavailableDate },
      { status: 201 },
    );
  } catch (error) {
