...
async function getIds(params: Promise<{ id: string; imageId: string }>) {
  ...
  return Number.isInteger(carId) && carId > 0 && Number.isInteger(imageIdNumber) && imageIdNumber > 0
...
export async function DELETE(_request: Request, { params }: { params: Promise<{ id: string; imageId: string }> }) {
  try {
    ...
    if (!ids) return NextResponse.json({ status: 400, message: "Invalid car or image ID." }, { status: 400 });
    ...
    const image = await prisma.carImage.findFirst({ where: { id: ids.imageId, carId: ids.carId } });
    if (!image) return NextResponse.json({ status: 404, message: "Image not found." }, { status: 404 });
    ...
    return NextResponse.json({ status: 500, message: "Failed to delete car image." });
  }
}