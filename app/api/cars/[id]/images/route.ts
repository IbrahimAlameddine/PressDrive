...
export async function GET(
  ...
  try {
    ...
    return NextResponse.json({
      status: 500,
      message: "Failed to fetch car images.",
    });
  }
}
...
export async function POST(
  ...
  try {
    ...
    if (images.length >= 5)
      return NextResponse.json(
        { status: 409, message: "A car can have a maximum of 5 images." },
        { status: 409 },
      );
    ...
    try {
      ...
      return NextResponse.json(
        { status: 400, message: "A valid image URL is required." },
        { status: 400 },
      );
    }
    if (
    ...
    return NextResponse.json({
      status: 500,
      message: "Failed to create car image.",
    });
  }
}
